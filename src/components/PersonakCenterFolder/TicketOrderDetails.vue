      ...
      <header class="MyticketOrderDetailsWrapHeader">
        <div class="MyticketOrderDetailsWrapHeaderBox">
          <div class="MyticketOrderDetailsWrapHeaderImageBox">
            <img src="../../assets/img/huileyouLogo.jpg" width="200" height="60" @click="toHome"
            ...
            <div class="MyticketOrderDetailsWrapContentDetalisLeft">
              <div class="MyticketOrderDetailsWrapContentDetalisLeftTop">
                ...
                <div class="MyticketOrderDetailsWrapContentDetalisLeftParameter clearfix">
                  ...
                  <span>成人: ￥ {{ticketOrderDetail.tm_or_FullPrice}}x{{ticketOrderDetail.tm_or_TicketCount}}</span>
                  <span
                    v-show="ticketOrderDetail.ts_to_ChildCount">儿童: ￥ {{ticketOrderDetail.tm_or_ChildPrice}}x{{ticketOrderDetail.tm_or_TicketCount}}</span>
                </div>
                <div class="AllMoney">
                  <strong>订单总金额: ￥{{ticketOrderDetail.tm_or_SumPrice}}</strong>
                </div>
              </div>
              <div class="orderContacts">
                ...
                <ul class="orderContactsData">
                  ...
                  <li>身份证号: {{ticketOrderDetail.tm_oc_CertNo}}</li>
                </ul>
              </div>
            </div>
            <div class="MyticketOrderDetailsWrapContentDetalisRight">
              ...
              <div class="MyticketOrderDetailsWrapContentDetalisRightContent">
                ...
                <div class="pendingPayment">
                  <strong
                    :style="{display:payStatus?'block':'none'}">{{ticketOrderDetail.tm_or_PayState | getTicketStatus}}</strong>
                  <span class="time" :style="{display:payStatus?'none':'block'}">请在20分钟前完成支付 超时订单将关闭</span>
                  <span class="pendingPaymentAllMoney"
                        :style="{display:payStatus?'none':'block'}">总价:￥{{ticketOrderDetail.tm_or_SumPrice}}</span>
                  <!--<span class="pendingPaymentAllMoneyOut">还需支付：<em>￥12960</em></span>-->
                  <el-button class="button" :style="{display:payStatus?'none':'block'}"
                             @click="goPay(ticketOrderDetail)">去付款
                  </el-button>
                  <span class="cancelOrder" :style="{display:payStatus?'none':'block'}"
                        @click="cancelOrder(ticketOrderDetail.tm_or_OrderID)">取消订单</span>
                </div>
              </div>
