        ...
        <div class="HotelHomePageWrapHeaderCarouselFigure">
          <div class="block">
            <el-carousel height="550px" indicator-position="none">
              <el-carousel-item v-for="item,index in homeImageList" :key="index">
                <a href="javascript:;" :style="{background:'url('+item.ht_hii_ImageUrl+')'}"></a>
              </el-carousel-item>
            </el-carousel>
          </div>
        </div>
        ...
        <div class="HotelQuery">
          ...
          <div class="HotelQueryBox">
            ...
            <div class="HotelQueryInputBox clearfix">
              <input type="text" placeholder="酒店名称" v-model="searchName">
              <a href="javascript:;" @click="toHotelAll"></a>
            </div>
          </div>
        </div>
      </div>
            ...
            <ul class="findPlayTheSeasonPictureList clearfix">
              <li v-for="item,index in HotPlayList">
                <a href="javascript:;" @click="changeHotlImg(item)">
                  <img width="390" height="230" v-lazy="item.ht_ai_Image" :key="item.ht_ai_Image">
          ...
          <div class="selectedCharacteristicRecommend">
            ...
            <ul class="selectedCharacteristicRecommendPictureList clearfix" ref="recommendType">
              <li v-for="item,index in characteristicRecommendList" :key="index"
                  @click="changeRecommendType(item,index)" @mouseover="hoverRecommendType(index)"
                  @mouseout="outRecommendType(index)">
            ...
            <ul class="exploreSeasonHotPictureList clearfix">
              <li v-for="item,index in hotDestinationList" @click="changeHotDestination(item)">
                <img v-lazy="item.ht_hl_HotelImage"
                     ...
                     height="250" :key="item.ht_hl_HotelImage">
    ...
    methods: {
      ...
      changeHotlImg(item) {
        const {href} = this.$router.resolve({
          ...
          query: {keycode: '', cityId: item.sm_af_AreaID}
        });
      ...
      changeHotDestination(item){
       sessionStorage.setItem('JDtitle','酒店_'+ item.ht_hl_HotelName)
      ...
      changeRecommendType(item, index) {
        sessionStorage.setItem('JDtitle','酒店_'+ item.ht_it_Name)

        if (index == 0) {

          const {href} = this.$router.resolve({
            name: 'HotelQingLv',
            params: {id: item.ht_it_ID},
          });
          window.open(href, '_blank')
        }
        if (index == 1) {
      ...
      getCity() {
        return new Promise((relove, reject) => {
          $.getScript('http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=js&ip=' + returnCitySN["cip"], () => {
            let city = remote_ip_info.city + '市'
            let options = {
              "loginUserID": "huileyou",
              "loginUserPass": "123",
              "operateUserID": "操作员编码",
            ...
            postPromise('http://webservice.1000da.com.cn/HotelWebPage/SelectCity', options)
              .then(data => {
      ...
      initHomeImage() {
        ...
        this.$store.dispatch('initHomeImage', selectHotelIndexImageInfo)
          .then(() => {
          }, err => {
            this.$notify({
              message: err,
